...
Профиль `terse` также не включён: в дополнительном A/B замере ускорения нет.
Проверка качества CLI теперь учитывает неверные маршруты и языки, не только API errors.
