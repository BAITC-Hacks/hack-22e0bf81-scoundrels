...
- [x] Реальный live eval 104 реплик: primary 1.000, full/ordered 0.990,
      multi-intent recall 0.962; 104/104 запросов без ошибок, conservative $0.325794
...
После eval исправлено только общее правило полноты составного запроса: явно запрошенный
следующий сервис не теряется, если необходимый предыдущий процесс начинается той же репликой.
Повторный платный eval автоматически не запускался; требуется отдельное решение и бюджет.
