...
Ветка: feat/router
Статус: AI Router готов к интеграции; полный live dev eval выполнен, общее правило
полноты multi-intent уточнено, повторный платный eval после изменения не запускался.
...
Это smoke из 5 авторских реплик, а не доказательство общей точности. На его момент полный
dev eval ещё не был выполнен; holdout и mouth-to-ear latency не измерены.
Артефакты отчёта локальные и игнорируются Git.
...
### Полный live dev eval 2026-09-23

Модель: `gpt-6-sol`, 104 dev-реплики, один запрос на реплику, retries=0.
Ограничители: `--max-requests 104 --max-usd 1.5`; failures=0, выполнено 104/104.
Ставки перед запуском сверены с официальной страницей модели: input $2.00,
cached input $0.20, cache write $2.50, output $10.00 за 1M токенов.

- Primary accuracy: 1.000; full match: 0.9904; ordered exact match: 0.9904.
- Multi-intent recall: 0.9615.
- RU: n=52, primary/full/ordered=1.000, recall=1.000.
- KK: n=45, primary/full/ordered=1.000, recall=1.000.
- Mixed: n=7, primary=1.000, full/ordered=0.8571, recall=0.750.
- Single: n=84, primary/full/ordered=1.000.
- Multi-intent: n=13, primary=1.000, full/ordered=0.9231, recall=0.9615.
- Out-of-scope: n=4, primary/full/ordered=1.000.
- Unclear: n=3, primary/full/ordered=1.000.
- Usage: input 881,044, из них cached input 876,720; uncached input 4,324;
  output 13,964. Все ответы получены с attempts=1.
- Консервативный ledger: $0.325794 из $1.50. Пересчёт observed usage по input,
  cached input и output: $0.323632; окончательное списание определяет OpenAI.
- Router latency: p50 2,917 ms; p95 4,561 ms. Все запросы получили cached tokens.

Одна ошибка: primary сценарий выбран верно, но в mixed составном запросе пропущен
второй явно запрошенный сервис. Это граница между регистрацией основного случая и
следующим зависимым сервисом: модель восприняла второй запрос как деталь первого из-за
отсутствия уже зарегистрированного процесса. Конкретные фразы и соответствия с ID в
prompt/code не добавлялись. Общее правило prompt уточнено: если предыдущий процесс
инициируется и зависимый сервис явно запрашивается в одной реплике, вернуть оба intent;
отсутствующие номера и слоты не должны стирать второй intent.

После этого изменения полный платный eval не повторялся. Для подтверждения изменения
нужен ещё один отдельный прогон 104 реплик. По первому расходу достаточно hard cap $0.50;
рекомендуемый cap $0.60 оставляет запас на изменение длины output, максимум 104 запроса,
retries=0. Повторять прогон следует только по явному разрешению.
После prompt-fix бесплатный набор повторён: 112 passed; платных запросов при этом не было.

## Что осталось

1. Интегратор подключает provider, budget, исключения, язык и ответ/исполнитель.
2. При необходимости подтвердить prompt-fix повторным прогоном 104 dev-реплик:
   рекомендуемый hard cap $0.60, максимум 104 запроса, retries=0. Первый прогон уже выполнен.
